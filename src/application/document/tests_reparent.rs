...
fn test_apply_reparent_updates_parent_child_edges() {
    ...
    let had_old_edge = doc
        .mindmap
        .edges
        .iter()
        .any(|e| e.edge_type == "parent_child" && e.to_id == source);
    ...
    let parent_edges: Vec<&MindEdge> = doc
        .mindmap
        .edges
        .iter()
    ...
    if had_old_edge {
        ...
        assert_eq!(
            parent_edges[0].from_id, new_parent,
...
fn test_apply_reparent_multiple_nodes_become_siblings() {
    ...
    let second_source = doc
        .mindmap
        .nodes
        .keys()
        .find(|k| {
            **k != new_parent && **k != first_source && !doc.mindmap.is_ancestor_or_self(k, &new_parent)
        })
...
fn test_apply_reparent_rejects_cycle() {
    ...
    assert!(
        undo.entries.is_empty(),
        "cycle should be rejected, no entries in undo data"
    );
