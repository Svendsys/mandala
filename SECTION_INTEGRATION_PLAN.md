...
| N4-A | **TextRun manipulation primitives in baumhard.** Foundation tier for N4 (per-grapheme range targeting). New module `lib/baumhard/src/mindmap/model/text_run_ops.rs` with four pure helpers: `find_run_containing(runs, idx)` (linear scan with sorted early-out, returns index of run whose `[start, end)` contains the grapheme idx; respects half-open semantics so `idx == run.end` is past the run); `find_run_starting_at(runs, idx)` (locates a run by exact `start == idx` — pairs with `split_at` for the right-half lookup); `split_at(runs, idx)` (carves a boundary by cloning the straddling run, sets the original's `end = idx` and the clone's `start = idx`, inserts the right-half; no-op on existing boundary, gap, or past-end); `slice(runs, start, end)` (clones every intersecting run with `start`/`end` clamped to the slice bounds, preserves internal gaps, original-coordinate output for picker `current_color_at`-over-range queries); `merge_adjacent_equal(runs)` (single-pass swap-write coalescer that joins neighbours sharing every style attribute AND meeting at a common boundary — gap-separated equals stay separate per the format spec since gaps carry semantic meaning). All four helpers preserve the format invariants from `format/text-runs.md`: sorted ascending, half-open `[start, end)`, no overlaps, gaps allowed. **25 new tests** in `text_run_ops::tests`: 4× find_run_containing (inside hits, half-open exclusion, gap miss, empty-vec none); 2× find_run_starting_at (exact boundary, non-boundary none); 6× split_at (inside-run insert, left-boundary noop, right-boundary noop, in-gap noop, past-end noop, find-right-half-by-start composition); 5× slice (clip to bounds, preserve gaps, empty when start>=end, empty when range in gap, full-range clones); 6× merge_adjacent_equal (coalesce matching, preserve mismatched, preserve gap boundary, chain three, partial chain, empty/single noop); 2× round-trip integration (split-mutate-merge for new attribute, split-no-mutate-merge re-coalesces). N4-B (range-aware document setters + console parser hooks) and N4-C (`SelectionState::SectionRange` + editor shift-select) build on this foundation. Workspace 2207 → 2232 tests. | ✅ |
