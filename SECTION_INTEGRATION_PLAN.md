...
| N4-C.a | **`SelectionState::SectionRange` variant + trait dispatcher plumbing.** Adds `SelectionState::SectionRange { sel: SectionSel, range: (usize, usize) }` — the editor's shift-select anchor will lift the cursor + anchor pair into this variant on close (N4-C.b). Per-section verbs route through the new `selected_range()` accessor + the existing `selected_section()` accessor (extended to expose the inner `SectionSel` for both `Section` and `SectionRange`); accessors that only care about the owning node (`is_selected`, `selected_ids`, `dedup_owning_node_ids`) treat it identically to `Section`. **Trait dispatcher plumbing**: `TargetId::Section` and `TargetView::Section` gain an `Option<(usize, usize)>` `range` field; `selection_targets` fans `SectionRange` to one `TargetId::Section { range: Some(_), .. }`; the `HasTextColor` and `AcceptsFontFamily` impls on `TargetView::Section` consult `range` and route to the `_range` setters when present, otherwise to the whole-section setters (back-compat for `Section` / `MultiSection`). `HandlesCopy` / `HandlesPaste` / `HandlesCut` ignore `range` for now (whole-section semantics; range-aware paste deferred to N4-D). **10 non-exhaustive match sites** updated; each treats `SectionRange` like `Section` for owning-node resolution. The architectural reviewer's "collapse `Section` → `SectionRange{full}`" suggestion was sidestepped — additive variant kept the blast radius bounded. **8 new tests**: 6 `SectionRange` accessor tests in `tests_selection.rs` + 2 in `console/traits/tests.rs` (fan-out carries range, end-to-end dispatcher routes to range-aware setter). Workspace 2266 → 2276 tests. | ✅ |
