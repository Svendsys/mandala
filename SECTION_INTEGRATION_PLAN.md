...
| N2 | **Auto-fit shrink path / `node fit-to-content` verb.** Shipped — pure `compute_one_node_text_floor` extracted from the existing `grow_one_node_to_fit_text` so callers can read the floor without max-wins-grow side effects; new `fit_node_to_content(node_id)` setter writes the floor unconditionally + runs `grow_one_node_to_fit_border` (so the new size has room for the rendered border) and pushes one `EditNodeAabb` undo entry. New `node fit-to-content` console verb (lifts to owning node from a Section selection). Pinned-section-size contribution to the floor survives — a `Some(size)` section contributes its own AABB as a lower bound, so fit-to-content on a node with pinned sections lands at the larger of measured-text and the union of pinned section AABBs. **8 new tests**: 4 setter + 4 verb (shrink, idempotent no-op, unknown-node, pinned-section-floor, oversized + verb-side counterparts). | ✅ |
