...
| — | **Deferred from PR1 review (intentional, separate iteration each):** (i) Handle-tree-builder unification: three reviewers across three rounds flagged `edge_handle` / `section_resize_handle` / `node_resize_handle` triplication (~600 LoC); a generic `HandleElement` trait + shared layout/build fns. Single-commit refactor, deferred to keep this fix-up scoped to correctness. (ii) Cross-platform parity: every new gesture (`MovingSection`, `SectionResize`, `NodeResize`, `node fit`) is native-only; WASM input path needs sibling implementations. Deferred — separate iteration each. (iii) `nodes/mod.rs` file split: 1481 lines, no longer tracks concept; split into `section_setters.rs` / `node_aabb.rs` / `payload.rs` is busywork rename, deferred. (iv) `Action::SetSectionOffset` / `SetSectionSize` / `SetNodeAabb` / `FitNodeToContent` parametric variants — funnel asymmetry, single-commit close, deferred until next gesture iteration. (v) Setter validation triplication (`set_section_offset` / `_size` / `_aabb`): collapse to `set_section_aabb` canonical + thin wrappers; deferred. | ⏳ |
