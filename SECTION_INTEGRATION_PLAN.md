...
| N1.1 | **Tier 2C-N1 review fix-up.** Three Opus reviewers (correctness / conventions / cross-cutting) flagged three Major bugs all addressed in one commit: (a) `apply_node_resize_to_tree` walked all descendants instead of `Flag::SectionRoot` children, so resizing a parent visually translated every descendant subtree mid-drag — fixed to mirror `apply_delta_node_and_sections`'s sections-only walk; (b) `set_node_size` / `set_node_aabb` skipped `grow_one_node_to_fit_text` / `_border` so the next unrelated edit silently bounced shrunk nodes back up — both setters now invoke the floor-respect pass like every other size-affecting setter, with the documented "shrink rejected up-front by the floor" semantic until Tier 2C-N2 lands auto-fit-shrink; (c) the multiplicative astronomical-typo guard (>100× prior) silently rejected legitimate gesture releases that grew a tiny node by many factors — replaced with a fixed absolute `MAX_NODE_AXIS = 1_000_000.0` ceiling, factored into a shared `check_node_size_typo` helper called from both setters. Plus 4 review-flagged tests: NE / SW `resolve()` cases, non-zero `position_delta` descendant-filter pin (asserts a child mind-node's tree position is unchanged after a node-resize translation), tiny-target floor-respect pin. | ✅ |
| N2 | **Auto-fit shrink path / `node fit` verb.** Shipped — pure `compute_one_node_text_floor` extracted from the existing `grow_one_node_to_fit_text` so callers can read the floor without max-wins-grow side effects; new `fit_node_to_content(node_id)` setter writes the floor unconditionally (after routing through `validate_node_size` + `check_node_size_typo` so a pinned-section typo can't bypass the absolute ceiling), runs `grow_one_node_to_fit_border` (so the new size has room for the rendered border), and pushes one `EditNodeAabb` undo entry — gated on the *post-border-grow* size against `before_size` so framed-node idempotency holds. New `node fit` console verb (single-token; lifts to owning node from a Section selection). Pinned-section-size contribution to the floor survives — a `Some(size)` section contributes its own AABB as a lower bound, so `node fit` lands at the larger of measured-text and the union of pinned section AABBs. Ambient auto-shrink is deliberately not part of the design (`format/sections.md` "size as floor" paragraph documents the asymmetry). | ✅ |
| N2.1 | **Tier 2C-N2 review fix-up.** Three Opus reviewers found 1 Critical (NaN slipping past `<= 0.0` guard — `f64::NAN <= 0.0` is false; self-fixed in commit `7642ebe` before reviews landed) and 2 Major: (a) framed-node idempotency was broken — `grow_one_node_to_fit_border` after the size write pulled `n.size` past the bare text floor, so the no-op gate (which compared against the bare floor) missed on every call after the first, stacking `EditNodeAabb` undo entries; gate now runs against the *post-border-grow* size, so framed nodes are idempotent. (b) `fit_node_to_content` skipped `validate_node_size` + `check_node_size_typo` that every other node-size setter runs — a pinned `section.size` of `5_000_000` propagated through the floor and bypassed N1's absolute ceiling; both checks now run on the floor-derived candidate before the write. Plus minor cleanups: `node fit-to-content` → `node fit` (single-token subverb idiom), trimmed doc comments on `fit_node_to_content` + `compute_one_node_text_floor`, em-dash → semicolon in error message, dead `let before` capture removed, dropped over-explanatory tail-comment. **5 new tests**: framed-node idempotency, unmeasurable-floor `Err` path, astronomical-pinned-section rejection, plus 2 direct unit tests for `compute_one_node_text_floor` (pinned-size dominance, non-finite-offset skip). | ✅ |
| PR1 | **PR-wide 5-Opus review round + fix-up.** A 5-reviewer round (architecture, gesture pipeline, setters, conventions, test coverage) covered the entire audit (`1bbceb7..HEAD`) end-to-end. **5 Critical correctness bugs** addressed in one fix-up commit: (a) `EdgeLabel` threshold-cross promotion captured `prev = doc.selection.clone()` *after* the new selection was assigned, so `rebuild_after_selection_change` always read prev == new and picked scene-only — leaving a `Single(node)` highlight stuck cyan through the drag. Hoisted the capture above the assignment. (b) Specific-gesture promotion arms (`hit_edge_label` / `hit_portal_label` / `hit_edge_handle` / `hit_node_resize_handle` / `hit_section_resize_handle`) called `.take()` before validation; on validation miss (deleted node, missing edge) execution fell through to subsequent arms and ultimately the empty-canvas pan fallback. Each arm now `return`s on validation miss so the promotion aborts honestly rather than silently pivoting from "resize" to "move" or "pan". (c) `MovingSection::drain` mutated section position but never refreshed the section-resize-handle tree; handles froze in place while the section visibly slid under them. The drain now reads the current section AABB and re-emits the handle slice through `update_section_resize_handle_tree_from_slice`. (d) `set_node_size` and `set_node_aabb` had the same framed-node idempotency bug N2.1 fixed for `fit_node_to_content` — pre-mutation no-op gate compared against `new_size`, but `grow_one_node_to_fit_border` could inflate post-write past `new_size`. Repeated identical calls stacked undo entries on framed nodes. Both setters now compare post-grow `n.size` against `before_size`. (e) Animation `start_ms` advanced in wall-clock time during tree-mutating drags (which suppressed `drain_animation_tick`); on release the next tick observed `elapsed >= total` and snapped the animation to its `to` state. New `MindMapDocument::shift_active_animations_start_ms` helper + `InitState.anim_pause_start_ms` field track the suppression interval and shift each animation's `start_ms` forward by the drag duration on release, so animations resume from where they froze. **6 new tests** pin each fix: framed-node idempotency for `set_node_size` + `set_node_aabb`, animation-shift advances lerp + zero-shift no-op, the existing idempotency tests now seed an above-floor target so the post-grow gate is exercised. Plus drift cleanup: `DragState` doc-comment "four-variant" → seven, `event_cursor_moved.rs` module header lists current variants, stale "100×" claim in `console/commands/node.rs` updated, stale `set_node_text` doc-block at `nodes/mod.rs:99-110` removed. | ✅ |
| PR2 | **Handle-tree-builder unification.** Closes the largest item from PR1's deferred list — three independent reviewers across three review rounds (Tier 2B-resize / 2C-N1 / 2C-N2 / PR-wide) flagged the byte-identical triplication of `edge_handle` / `section_resize_handle` / `node_resize_handle` tree-builders. New `HandleVisual` trait in baumhard exposes the per-instance fields (`position`, `glyph`, `color`, `font_size_pt`, `channel`); the three `EdgeHandleElement` / `SectionResizeHandleElement` / `NodeResizeHandleElement` types impl it next to their definitions in `scene_builder/`. New generic `build_handle_tree<E: HandleVisual>` / `build_handle_mutator_tree<E>` / `handle_identity_sequence<E>` in `tree_builder/handle.rs` replace the three near-identical specialized files (`tree_builder/{edge_handle,section_resize_handle,node_resize_handle}.rs`) which are deleted entirely. App-side `update_*_handle_tree*` family collapses through one new `update_handle_canvas_role<E>(role, elements, app_scene)` helper that owns the `CanvasDispatch` / `register_canvas` / `apply_canvas_mutator` shape — the per-role wrappers each route to it with their `CanvasRole`. **Net: 262 lines deleted, zero behavior change** (workspace test count unchanged at 2168). `edge_handle_channel_for` (the `EdgeHandleKind → channel` mapper) moves from `tree_builder/edge_handle.rs` to `scene_builder/edge_handle.rs` next to the `HandleVisual::channel` impl that calls it, since the test in `tree_builder/tests/edge_handle.rs` references it directly through the new path. | ✅ |
| — | **Deferred from PR1 review (intentional, separate iteration each):** (i) ~~Handle-tree-builder unification~~ closed by PR2. (ii) Cross-platform parity: every new gesture (`MovingSection`, `SectionResize`, `NodeResize`, `node fit`) is native-only; WASM input path needs sibling implementations. Deferred — separate iteration each. (iii) `nodes/mod.rs` file split: 1481 lines, no longer tracks concept; split into `section_setters.rs` / `node_aabb.rs` / `payload.rs` is busywork rename, deferred. (iv) `Action::SetSectionOffset` / `SetSectionSize` / `SetNodeAabb` / `FitNodeToContent` parametric variants — funnel asymmetry, single-commit close, deferred until next gesture iteration. (v) Setter validation triplication (`set_section_offset` / `_size` / `_aabb`): collapse to `set_section_aabb` canonical + thin wrappers; deferred. | ⏳ |
| N3 | **`SelectionState::MultiSection`.** Shipped — new `MultiSection(Vec<SectionSel>)` enum variant + `from_sections` constructor (mirrors `from_ids`'s empty/single/many fold) + `selected_sections() -> &[SectionSel]` accessor. Shift+click on a section extends Section ↔ MultiSection in `app/click.rs`: each shift+click toggles the targeted section in/out of the set; cross-node section sets are legal; a re-click on the only selected section toggles back to `None`. `selection_targets` fans `MultiSection` out to one `TargetId::Section` per entry, so per-section verbs (colour text, font size / family) apply to every section. Tree-highlight pipeline (`scene_rebuild.rs::selection_highlight_entries`, `event_cursor_moved.rs::rebuild_selection_highlight`, `click.rs::rebuild_all_with_mode`) routes Section/MultiSection through one entry per `(node_id, section_idx)` so multiple sections of the same node can each tint independently. Resize-handle emission (the `selected_section: Option<(&str, usize)>` field on `SceneSelectionContext`) gates only on the single-section `Section` variant — `MultiSection` deliberately emits no handles, and the threshold-cross promotion arm path is unchanged. **10 new tests** in `document/tests_hit_move.rs` + `console/traits/tests.rs`: `from_sections` empty/single/many cases, `is_selected` matches any section's node, `selected_ids` dedup'd per node, `selected_section()` returns None for MultiSection, `selected_sections()` returns the slice, single-Section returns singleton slice, other variants return empty; plus `selection_targets` fans out per-entry. Per-iteration consumers updated: `font` verb (size+min+max), `zoom` verb (dedup'd to owning nodes), `border` verb (dedup'd to owning nodes), color picker hint shape, color picker target resolution, click.rs section-aware shift+click, topology delete (dedup'd to owning nodes for the bare-Delete keystroke). | ✅ |
| N3.1 | **Tier 2C-N3 review fix-up.** Three Opus reviewers found 1 Critical + 8 Major bugs all addressed: (a) **Critical clipboard data loss** — `apply_copy_or_cut` broke on first target; only the first section's text reached the OS clipboard for `MultiSection` cut/copy. The loop now iterates every Section target, joins texts with `\n` for the OS clipboard, and writes the structured payload only when count == 1 (multi-section structured paste deferred until `ClipboardContent::MultiSection`). (b) **Major: `from_sections` doesn't dedup** — downstream consumers assumed uniqueness; duplicates inflated fan-out counts. The constructor now dedup's by `(node_id, section_idx)` in first-seen order. (c) **Major: `targets_kind_label` singular for multi-section** — `Section` arm now pluralizes via `len() > 1` like the `Node` arm. (d) **Major: highlight pipeline 3x duplication** — `scene_rebuild::selection_highlight_entries` promoted to `pub(in crate::application::app)`; both inline copies in `click.rs::rebuild_all_with_mode` and `event_cursor_moved.rs::rebuild_selection_highlight` now call the helper. (e) **Major: dedup-owning-node-ids 4x duplication** — new `SelectionState::dedup_owning_node_ids()` method; `border`, `zoom`, `topology` Delete all route through it. (f) **Major: color picker contextual commit single-handle** — Section commit arm now reads `doc.selection`; for `MultiSection`, fans out across every selected section. (g) **Major: section-drag promotion overwrites MultiSection** — `already_selected` check now treats the pressed section as already-in-set when MultiSection contains it; preserves the multi-set during drag. (h) **Major: whole-node-drag demote misses MultiSection** — `needs_demote` extended to MultiSection containing the pressed node. (i) **Major: `section move/resize` rejection message** clearer for MultiSection ("multi-section selection — single-target only; pass section=<idx> or click one section first"). **3 new tests** pin the fixes: `from_sections` dedup, all-duplicates collapse, `dedup_owning_node_ids` across variants. | ✅ |
| N3.2 | **Tier 2C-N3.1 review fix-up.** Three Opus reviewers found 5 class-parallel data-loss bugs the N3.1 fix-up missed plus 3 Major UX/state bugs and a broad test-coverage gap; all addressed. **(a) Critical: paste-path mirror.** `apply_paste` rewrote the same OS-clipboard blob to every section target — symmetric to the N3.1 copy-side bug it fixed. New `split_paste_for_targets(text, target_count)` returns `Some(fragments)` when the split count matches target count (round-trip from a Mandala multi-target copy zips 1:1) and `None` to broadcast (cross-app paste). **(b) Critical: separator collision.** The N3.1 fix joined section texts with `\n`, but a section's text can contain intra-section `\n` line breaks from the inline editor — paste-side split would over-fragment. Joiner moved to `MULTI_TARGET_SEPARATOR = "\n\n"`. **(c) Critical: stale `SECTION_BUFFER`.** Multi-section copy doesn't write the structured buffer (no MultiSection payload variant) — a leftover single-section payload from a prior copy survived to win the byte-equal probe on the next paste. New `clipboard::clear_section_clipboard()` is called up-front in `apply_copy_or_cut`. **(d) Critical: color picker Node arm not fanned out.** The N3.1 fix only extended the Section arm's fan-out; `Multi(ids)` selection + Node-handle commit silently dropped every node except the bound handle. New `node_commit_targets` helper drives fan-out across `Multi`. **(e) Critical: clipboard Text arm first-wins.** `apply_copy_or_cut`'s text-content path took the first target's text and broke; for `Multi(ids)` (each Node target produces `display_text`) only the first node's text reached the clipboard. Now accumulates every text payload and joins with `MULTI_TARGET_SEPARATOR`. **(f) Major: shift+drag scope shrinks via demote-first ordering.** The whole-node-drag arm demotes `MultiSection` to `Single(node_id)` *before* the shift+drag harvest read `selected_ids()`, dropping every other node from the drag scope. New `pre_demote_ids` snapshot taken before the demote runs. **(g) Major: color picker MultiSection commit drops handle when selection diverged.** When the user changed selection between picker open and commit, the commit fanned out across the new `MultiSection` set without including the bound handle's section — silently dropping the bound section. New `section_commit_targets` helper unions the handle into the set with dedup. **(h) Major: section-drag preserve→demote.** N3.1's preserve-MultiSection-on-section-drag created a UX incoherence (mid-drag picker hint reads "MultiSection" while user moves a single section). Reverted to demote-on-press, mirroring the whole-node arm. **(i) Test coverage.** N3.1 had 3 tests for 9 fixes. N3.2 adds **25 new tests**: split_paste_for_targets (5 cases — separator, single-target, matching/mismatched count, empty fragments), apply_copy_or_cut buffer behavior (3 — stale clear, single-section structured write, multi-section no-payload), `selection_after_section_drag_press` / `selection_after_node_drag_press` (8 — preserve-on-exact, demote-multisection, rewrite-on-different, single-no-op, multi-no-op, demote-section, demote-multisection-to-single, replace-when-not-selected), pre-demote snapshot pin (1), `node_commit_targets` / `section_commit_targets` (6 — Multi fan-out, Single bound-only, fallback when diverged, MultiSection fan-out + handle-union, Section no-dup, fallback for other states). Drive-by: `dedup_owning_node_ids` order-preservation test added, two new pure helpers (`selection_after_*_drag_press`) extracted from inline match arms in `event_cursor_moved.rs` so the demote logic is no longer test-only-in-the-renderer-stack. | ✅ |
| — | **N3 deferred (separate iteration each):** (i) **WASM port** — shift+click section extension is desktop-only; WASM path needs the equivalent shift handling in `run_wasm/event_mouse_click.rs`. (ii) **`ClipboardContent::MultiSection`** — multi-section structured paste fan-out (today multi-section copy falls back to text-only on within-app paste). (iii) **`Action::SetSectionSelection`** parametric — keybind / macro / palette path for "extend selection by section". (iv) **Selection-nav verbs** (`invert_selection`, `select_parent` / `_child` / `_sibling`) silently no-op for MultiSection — collapse-to-first or fan-out-each is a small follow-up. (v) **`Action::EditSelection` on MultiSection** — `apply_open_text_edit_on_single` falls through silently for MultiSection (only Single + Section route to the editor); collapse-to-first-section or surface clearer error. (vi) **Picker live preview fan-out for Node / Section handles** — `apply_picker_preview` only previews Edge handles today; commit fans out correctly post-N3.2 but live preview still only repaints the bound handle. (vii) **Grouped undo across fan-out** — `commit_color_picker` / `apply_copy_or_cut` cut / clipboard paste each push N undo entries when fanning out across `Multi` / `MultiSection`; user's single Ctrl+Z only reverts the last target. Will be addressed by `UndoAction::Group` when that lands. (viii) Minor: `font` verb formatting helper unification (Multi-node + MultiSection arms 95% duplicated); `click.rs` indentation drift after the if-else split; standalone-picker hint string drift. | ⏳ |
| N3.3 | **Tier 2C-N3.2 review fix-up.** Three Opus reviewers found zero Critical/Major correctness bugs in N3.2 — the round is converging. The cross-cutting reviewer flagged one Minor symmetry gap and one doc drift; conventions reviewer flagged test-naming + test-placement deferrals. All addressed: **(a) Symmetry: section/node resize arms now demote MultiSection on promotion** — the same demote-on-press the section/node move arms got in N3.2 was missing on the resize arms. Both arms now invoke `selection_after_section_drag_press` / `selection_after_node_drag_press` (the existing extracted helpers drop in trivially) so the mid-drag picker hint reflects the in-flight resize gesture's actual target rather than the prior multi-set. **(b) Doc drift: CONCEPTS.md + format/sections.md** still claimed N3.1's preserve-MultiSection semantics for section drag; both updated to reflect demote-on-press and the resize-arm parallel. **(c) `node_commit_targets` now dedups Multi(ids)** — `from_ids` doesn't enforce uniqueness, and a stale duplicate would silently produce a redundant setter call (idempotent on colour but doc-state churn / extra undo work). **(d) Test placement: 16 SelectionState tests** moved from `tests_hit_move.rs` to `tests_selection.rs` (closing the deferred-row that called this drift out — §5 forbids deferring drive-by cleanup when the file is being touched). **(e) Test naming: 24 N3.2 tests** prefixed `test_` per TEST_CONVENTIONS §T3 (also covers reviewer's `node_drag_press_demotes_same_node_section` → `_section_to_single` rename request). **(f) Helper API: `clear_section_clipboard` / `clear_section_clipboard_for_tests`** were byte-identical after N3.2; the test-only suffix removed and 9 callers updated to use the unified function. **(g) Doc-comment trim: `pre_demote_ids` (11 lines → 4), `MULTI_TARGET_SEPARATOR` worked-example sentence cut, `apply_copy_or_cut` two preamble paragraphs merged into 2 lines** per §8 WHY-not-WHAT. **(h) `MULTI_TARGET_SEPARATOR` const hoisted** to top of file (was wedged between two consumers). **(i) End-to-end multi-section copy→paste round-trip test** added — the helper-level tests pin each leg in isolation, the new test fills the integration gap by driving copy → split → paste against a real `MultiSection` selection. **1 new test net** (round-trip; relocations are net-zero). Workspace 2206 → 2207. | ✅ |
| N4-A | **TextRun manipulation primitives in baumhard.** Foundation tier for N4 (per-grapheme range targeting). New module `lib/baumhard/src/mindmap/model/text_run_ops.rs` with five pure helpers (six after N4-A.1's `insert_run` addition): `find_run_containing(runs, idx)` (linear scan with sorted early-out); `find_run_starting_at(runs, idx)` (paired right-half lookup for `split_at`); `split_at(runs, idx)` (carve boundary; no-op on existing boundary, gap, or past-end); `slice(runs, start, end)` (clamped clones in original coordinates, preserves gaps); `merge_adjacent_equal(runs)` (single-pass swap-write coalescer — gap-separated equals stay separate per format spec). All preserve `format/text-runs.md` invariants. **25 new tests**: find_run_containing × 4, find_run_starting_at × 2, split_at × 6, slice × 5, merge_adjacent_equal × 6, round-trip × 2. Workspace 2207 → 2232 tests. | ✅ |
| N4-A.1 | **Tier 2C-N4-A review fix-up + PR-wide cross-tier bugs.** Five Opus reviewers (2 PR-wide + 3 N4-A focused) found 1 Critical + 3 Major across the cumulative PR plus polish on N4-A itself. **(a) Critical: paste broadcast leaks structured payload across MultiSection targets.** Single-section copy seeds `SECTION_BUFFER` with a `SectionPayload`; subsequent `MultiSection` paste falls through to broadcast (count mismatch, no `\n\n` in single-fragment text); each per-target `clipboard_paste` byte-equal probes the still-populated buffer and applies the same payload (offset, size, channel, trigger_bindings) to every target — silent data corruption. `apply_paste` now clears the structured buffer when `targets.len() > 1 && fragments.is_none()`, gating only the broadcast path; single-target paste preserves the buffer for within-app section→section round-trips. **(b) Major: `Action::Cut` doesn't trigger rebuild.** `apply_copy_or_cut(is_cut, doc)` mutates section text on cut but the dispatcher (`action_core.rs`) called it without `with_doc_rebuild` — cut section's text stayed visible in the scene until next interaction. Split into `Action::Copy` (read-only) and `Action::Cut` (routed through `with_doc_rebuild` + `rebuild_after_geometry_change`). **(c) Major: N4-A foundation gap — missing gap-fill primitive.** N4-B's range-targeted setters need to fill an uncovered gap when `[range.start, range.end)` falls in one — `split_at` no-ops, `merge_adjacent_equal` has nothing to merge. New `insert_run(runs, run) -> usize` primitive in `text_run_ops` inserts a fresh run at the sorted position implied by its `start`, debug-asserts non-overlap, returns the insertion index. Pairs with `merge_adjacent_equal` for the post-insert coalesce. **(d) Major: `slice` / `split_at` shadow inherent `[T]` methods.** Cross-cutting reviewer flagged that glob-exporting these names (`pub use text_run_ops::*`) would let `runs.split_at(idx)` silently call the inherent method instead of our function. Resolution: keep namespaced (no flatten), document the call-site discipline in the module `//!`. **(e) Minor: `manual_let_else`** clippy lint in `split_at` fixed via `let Some(...) else { ... }`. **(f) Nit: `style_eq` over-narrating doc-comment** trimmed to one phrase ("Coalesce predicate for `merge_adjacent_equal`"). **(g) Doc drift: format/sections.md:170** claimed `set_section_size` + `set_section_offset` as the resize commit path; code uses the atomic `set_section_aabb` setter (introduced in Tier 2B-resize fix-up). **(h) Doc drift: format/schema.md MindSection table** omitted `trigger_bindings` field; added with cross-reference to `format/sections.md:48`. **(i) Plan tracker: N3.2 / N3.3 row order** corrected (N3.2 now precedes N3.3); test count for N3.3 fixed (1 net new, was claimed 2). **8 new tests**: insert_run × 5 (empty, into-gap, at-end, merge-with-neighbour, debug overlap panic), split_at idempotency, paste-broadcast guard × 2 (clears stale on multi-target, preserves on single-target). Workspace 2232 → 2240 tests. | ✅ |
| N4-B | **Range-aware document setters + `range=A..B` console kv.** Three new public document setters in `nodes/mod.rs` mirror the existing uniform-rewrite section setters with grapheme-range targeting: `set_section_text_color_range(node_id, section_idx, range_start, range_end, color)`, `set_section_font_size_range(...size_pt)`, `set_section_font_family_range(...family)`. All three route through a single new baumhard primitive `text_run_ops::mutate_in_range(runs, range_start, range_end, template_for_gap, mutate)` that composes split → mutate-in-range → gap-fill → merge: `split_at(range_start)` + `split_at(range_end)` to carve boundaries; `iter_mut` over fully-in-range runs to apply the closure; gap-detection walk to find uncovered spans inside `[range_start, range_end)` and `insert_run` (N4-A.1's primitive) to fill each with a clone of `template_for_gap`; finally `merge_adjacent_equal` to coalesce. **Cascade template construction** lives in a new private `clamp_range_and_build_template` helper on `MindMapDocument` — clones the section's first existing run (carries the section's effective font / size / style) when present, else builds from `node.style.text_color` + workspace defaults (`LiberationSans` / 24pt) baked into `defaults::default_orphan_node`. Caller overwrites the one attribute it's setting before passing the template to `mutate_in_range`. **No-op detection** via pre/post `text_runs` compare: an apparent change that doesn't actually mutate the run set unwinds the spurious undo entry. **Console kv parser** extended on `color` and `font` verbs: new shared `commands/range_kv.rs` module exposes `parse_range_kv("A..B")` rejecting empty halves / non-numeric / inverted ranges. `range=A..B` requires `section=N` (ranges target grapheme indices inside one section); without a section the verb errors with a clear usage message. `font set <family>` also accepts `section=N` + `range=A..B`, bypassing the trait dispatcher to call the per-section / range setter directly (the trait surface doesn't carry per-section / per-range routing today — deferred to N4-C alongside `SelectionState::SectionRange`). **Picker / shift+click trait dispatch routes are deferred to N4-C** alongside the new `SelectionState::SectionRange` variant. **23 new tests**: 8 in baumhard `text_run_ops::mutate_in_range` (inside-one-run, crosses-boundary-merges, fills-pure-gap, fills-partial-gap, exact-match-idempotent, empty-range-noop, fills-when-runs-empty, no-change-recoalesces); 6 in `commands/range_kv::tests` (happy-path, missing-separator, empty-halves, non-numeric, inverted-or-empty, negative-rejected); 9 in `tests_nodes.rs` (inside-one-run, no-op-no-undo, clamps-end, empty-returns-false, missing-section-returns-false, undo-round-trip, font-size-grow, font-family-in-range-only, fills-gap). Workspace 2240 → 2263 tests. | ✅ |
| N4-B.1 | **N4-B test fixture flake fix.** `test_set_section_text_color_range_fills_gap` failed in CI (`002bf18` push) because `pinned_two_section_node()` → `first_testament_node_id()` iterates a `HashMap<String, MindNode>` whose order isn't stable across runs (per-instance hash seed). Different orders picked testament nodes with different `section[0].text` lengths; the test's `total >= 8` assertion panicked when an order picked a short-text node. Fix: new `set_section_zero_text_and_single_run` test helper overwrites `section[0]` with a known 10-grapheme string before each range setter test runs. **Local repro: 3/5 fail without patch, 5/5 clean with it.** Applied to all 9 range setter tests. | ✅ |
| N4-B.2 | **Five-Opus full-PR review fix-up.** Three Major findings (no Critical), three Minor, plus doc-trim and plan-tracker hygiene. **(a) Major: `text_run_ops` no debug-asserts on input invariants** — silent-corruption risk if a future caller violates the sorted/non-overlapping precondition. New private `debug_assert_invariants(runs)` helper called from every public `pub fn` first line; zero release cost; 3 new `#[should_panic]` regression tests. **(b) Major: `SectionPayload` all-`pub`-fields forward-compat trap** — adding any field to `MindSection` breaks every clipboard call site. Added `#[non_exhaustive]` so external consumers must route through `from_section`. **(c) Major: `first_testament_node_id` HashMap-iteration-order flake source** — went deeper than N4-B.1's per-test fix. Helper now sorts keys and picks the lexicographic minimum, deterministic across all `HashMap` seeds. Same fix applied to `first_n_testament_node_ids` + `two_testament_node_ids`. Closes the entire class of flakes the test reviewer flagged across 5+ at-risk tests (`_grows_node_to_fit_*`, `_idempotent_*`, etc.). **(d) Minor: `set_section_font_size_range` NaN/Infinity guard** — `is_finite()` early-out before the `.round() as u32` cast that would otherwise saturate. **(e) Minor: `Action::Cut` conditional rebuild** — `apply_copy_or_cut` now returns `bool` for "any target accepted"; the dispatcher gates `rebuild_after_geometry_change()` on it, mirroring `apply_paste`. **(f) Minor: range-start-out-of-bounds error message** — `apply_section_colours` and `section_font_outcome` pre-flight check `range_start` against the section's grapheme count and surface "color: range_start=N is past the section's grapheme count (M)" instead of silent "no change". **(g) Minor: `parse_range_kv` visibility** tightened to `pub(super)` (only used inside `commands/`). **(h) Doc-comment trim** — verbose docs reviewer flagged `text_run_ops` module preamble + 5 helper docs (`mutate_in_range` algorithmic narration was the worst at 31 lines explaining a 4-step algorithm); + `nodes/mod.rs` "Range-targeted section setters (N4-B)" preamble comment; + `clamp_range_and_build_template` docstring. Net: ~80 doc-comment lines removed without losing a single load-bearing WHY. **Deferred to a future commit:** criterion benches for `text_run_ops` primitives (§B7 violation flagged by perf reviewer; primitives aren't on per-frame paths today, the bench scaffolding pairs naturally with N4-C's picker-preview wire); `tests_nodes.rs` / `tests_hit_move.rs` file splits (architectural reviewer); pure-logic test mods inside `commit.rs` / `lifecycle.rs` hoisting out of their `cfg(not(wasm32))` gate; range-setter triple-duplication consolidation via a sibling private helper. **3 new tests** ($\#$\[should_panic\] invariant guards). Workspace 2263 → 2266 tests. | ✅ |
| N4-C.a | **`SelectionState::SectionRange` variant + trait dispatcher plumbing.** Adds `SelectionState::SectionRange { sel: SectionSel, range: (usize, usize) }`. Per-section verbs route through the new `selected_range()` accessor + the existing `selected_section()` accessor (extended to expose the inner `SectionSel` for both `Section` and `SectionRange`). `TargetId::Section` and `TargetView::Section` gain an `Option<(usize, usize)>` `range` field; `selection_targets` fans `SectionRange` to one `TargetId::Section { range: Some(_), .. }`; `HasTextColor` and `AcceptsFontFamily` impls route to `_range` setters when present. **10 non-exhaustive match sites** updated. Architectural choice: additive variant (not "collapse `Section` → `SectionRange{full}`") — kept the compile-time blast radius bounded. **10 new tests** (6 accessor + 2 dispatcher fan-out + 2 end-to-end). Workspace 2266 → 2276 tests. | ✅ |
| N4-C.a.1 | **3-Opus review fix-up of N4-C.a.** All three reviewers (correctness, cross-cutting, integration) confirmed the architectural choice was right but flagged 1 Critical + 12+ Major missed sites — `if let`, `matches!`, and `_ => false` patterns slipped past the compiler. **Critical (latent until N4-C.b's editor anchor lands):** `Action::Cut` / `Paste` on `SectionRange` would silently destroy the whole section. Both `clipboard_cut` and `clipboard_paste` Section arms now return `NotApplicable` when `range.is_some()`, surfacing a clear console error instead of whole-section overwrite. (Range-aware copy is non-destructive; left as fall-through to whole-section semantics.) **Major (every site that pattern-matched `Section(_)` with a fall-through `_ => false`):** (a) `scene_rebuild::selection_highlight_entries` paints per-section for SectionRange (was whole-node fall-through). (b) `scene_rebuild::selection_change_touches_tree` matches `SectionRange` AND the previously-also-missed `MultiSection` so transitions trigger full rebuild not scene-only. (c) `apply_open_text_edit_on_single` accepts `SectionRange` (Enter / activate keystroke now reopens the editor). (d) `selection_after_node_drag_press` demotes `SectionRange` on whole-node press. (e) `invert_selection_in` / `select_parent_in` / `select_child_in` / `select_sibling_in` all route `SectionRange` through the `Section` arm (or via `selected_section()` for sibling). (f) `event_mouse_click::hit_section_resize_handle` + `document::mod.rs` scene-builder context use `selected_section()` so resize handles emit. (g) `commands/section.rs::resolve_section_idx` / `resolve_node_id` accept `SectionRange`. (h) `commands/node.rs::execute_fit` / `execute_resize` accept `SectionRange`. (i) Two inline match arms in `commands/font.rs::execute_font_set` accept `SectionRange`. (j) `commands/color.rs::apply_section_colours` accepts `SectionRange`. (k) `commands/mutation.rs::resolve_target_id` accepts `SectionRange`. (l) `color_picker_flow::section_commit_targets` includes `SectionRange`. **Minor:** symmetric NaN/Inf guard on `set_section_font_size` + `set_node_font_size` (was on the `_range` siblings only). **Doc drift:** `CONCEPTS.md` `SelectionState` section gains a `SectionRange` bullet describing the editor-close lift + clipboard-rejection contract. **Plan-tracker accuracy:** N4-C.a row's "8 new tests" claim was undercount; actual was 10. **4 new tests**: clipboard NotApplicable on SectionRange × 2 (Cut, Paste), `selection_after_node_drag_press` demotes SectionRange × 1, `select_parent_in` walks up from SectionRange × 1. Workspace 2276 → 2280 tests. | ✅ |
| N4-C.b.1 | **Picker handle range plumbing.** `ColorTarget::Section` and `PickerHandle::Section` gain an `Option<(usize, usize)>` `range` field — when the active selection is `SelectionState::SectionRange`, `picker_target_for` plumbs the sub-range through to the picker handle so commit lands on `set_section_text_color_range` instead of the whole-section setter. **`current_color_at` cascade over a range**: when `range.is_some()`, scans only the in-range runs via `text_run_ops::slice`; partial-coverage / non-unanimous in-range runs fall back to `node.style.text_color` (same cascade source the write-side range setter consults). **`commit_color_picker` Section arm** routes range-aware: when the bound handle carries a sub-range, calls `set_section_text_color_range` directly (skipping the MultiSection fan-out — sub-range semantics on a multi-section selection don't compose because different sections have different lengths). Whole-section / Section / MultiSection commits fan through `section_commit_targets` as before. **2 new tests**: picker target carries range from `SelectionState::SectionRange` selection × 1, `current_color_at` over range with unanimous-in-range / non-unanimous fallback × 1. Workspace 2280 → 2282 tests. | ✅ |
| N4-C.b.1.1 | **3-Opus review fix-up of N4-C.b.1.** Two Major + four Minor flagged across correctness / cross-cutting / integration. **(a) Major: `current_color_at` over a sub-range that crosses a gap returned the spanning run's color** — `text_run_ops::slice` doesn't expose gap holes, and the unanimous-filter trivially passed on a single clipped run. The picker seeded with the wrong color whenever the user's range partially covered a run. New coverage check: in-range runs must collectively span the full `[rs, re)` with no gaps for "unanimous" to be meaningful; else fall back to `node.style.text_color`. **(b) Major: silent Cut/Paste no-op on `SectionRange`** — the N4-C.a.1 commit message advertised "clear console error" but only the kv-shaped `apply_kvs` path surfaced it; the action-keybind path (`Action::Copy` / `Cut` / `Paste`) folded `NotApplicable` into a silent no-op. New `log_clipboard_skip_for_section_range` helper called from `action_core::{Action::Copy, Action::Cut}` and from `apply_paste`'s `else` branch when `any_applied == false`; emits `log::warn!` with a clear "range-aware clipboard deferred; clear range with arrow-without-shift" hint. **(c) Minor: CONCEPTS.md drift** — N4-C.a.1's bullet claimed "Copy / Cut / Paste reject" but Copy actually falls through to whole-section copy. Bullet rewritten to distinguish: Cut/Paste reject (destructive guard), Copy falls through (non-destructive). **(d) Minor: format/sections.md zero mention of SectionRange** — added a "Section sub-range selection" subsection covering both producers (verb `range=A..B` shipped, editor anchor deferred to N4-C.b.2), the clipboard contract, and the picker contract (cascade over range, MultiSection-fan-out skip, deferred preview). **(e) Minor: stale picker handle range silently no-ops** — when the section shrinks below the bound handle's range between picker open and commit, `set_section_text_color_range` returns `false` silently. Added `log::warn!` from `commit_color_picker` Section arm when the range-aware setter returns false. **4 new tests**: gap-coverage × 1 (range crossing a gap → fall back to default), pure-gap × 1 (range entirely uncovered → fall back), multi-run unanimous × 1 (`iter().all` branch with `len > 1`), `select_sibling_in` drops SectionRange to Section × 1 (pins the documented "fresh selection, not a range carry-over" semantic). **Deferred to N4-C.b.2 / N4-D:** `apply_picker_preview` Section preview wiring (commit-only today; same gap as Node arm), divergence path test (mid-flow MultiSection switch with bound SectionRange handle), `apply_open_text_edit_on_single` caret-anchor restoration on reopen. Workspace 2282 → 2286 tests. | ✅ |
| N4-C.b.2 | **Editor shift-select anchor.** Closes N4-C: the inline text editor now produces `SelectionState::SectionRange` from a user-driven shift-arrow / shift-home / shift-end gesture. New `TextEditState::Open.selection_anchor: Option<usize>` field tracks the grapheme position the user first held shift at. Six new `Action` variants (`TextEditCursor{Left,Right,Up,Down,Home,End}Select`) bound to `Shift+Arrow*` / `Shift+Home` / `Shift+End` by default. The shift-cursor handler seeds the anchor at the pre-action cursor position when first invoked, then moves the cursor while preserving the anchor; non-shift cursor actions and any text edit (delete / type) clear the anchor — range-aware editing (typing-replaces-selection) is deferred to a future tier. **Editor close → SectionRange lift**: a new pure helper `lift_anchor_to_section_range(anchor, cursor, node_id, section_idx)` in `editor.rs` returns `Some(SectionRange { sel, range: (min, max) })` when the anchor is set and differs from the cursor; `None` when unset or coincides (collapsed). `close_text_edit` calls the helper on the commit path only (cancel reverts the buffer, so a lift would point past post-revert grapheme counts) — see N4-C.b.2.1 fix-up. **10 new tests**: 6 anchor lifecycle (first shift-cursor seeds anchor, subsequent shift-cursor preserves anchor, non-shift collapses, text edits clear, shift-left seeds above cursor, shift-home seeds at line start) + 4 lift contract (anchor below cursor → SectionRange, anchor above cursor → SectionRange, anchor unset → None, anchor equals cursor → None). **Closes N4-C** — every consumer of `SectionRange` is correct (N4-C.a + N4-C.a.1) and both producers are wired (verb `range=A..B` from N4-B + editor anchor from this commit). N4-D (range-aware clipboard cut/paste) and the deferred picker preview wiring remain. Workspace 2286 → 2296 tests. | ✅ |
| N4-C.b.2.1 | **3-Opus review fix-up of N4-C.b.2.** Four Major + four Minor flagged across correctness / cross-cutting / integration. **(a) Major: literal-char typing path didn't clear `selection_anchor`** — `apply_text_edit_action` correctly cleared the anchor on text edits routed through it (`TextEditDeleteBack` / `Forward` / `WordBack` / `WordForward`), but printable-char typing flowed through `handle_text_edit_key`'s no-Action fall-through which mutated `buffer` / `cursor` / `regions` directly without touching the anchor. Result: a user could shift-select then type, and on close the lift would emit a `SectionRange` over graphemes that no longer matched the user's intent. Refactored the literal-char branch into a testable `apply_literal_char_insert(name, logical_key, state)` helper that clears the anchor in all three branches (`enter`, `tab`, printable chars including the combining-mark merge case). Tightened the printable-char body to call `grapheme_chad::insert_str_at_grapheme` directly instead of open-coding the byte-offset lookup + `String::insert_str` it wraps. **(b) Major: cancel path lifted a stale range** — `close_text_edit(commit=false, ...)` reverts the model to `original_text`, which can have fewer graphemes than the (anchor, cursor) pair was selected over; lifting a `SectionRange` past the post-revert grapheme count silently no-ops on every downstream consumer. Gated the lift behind `if commit { ... }` so cancel preserves the pre-edit `Section` selection. **(c) Major: `dispatch_compatible` missed the 6 new `*Select` variants** — the existing TextEdit cursor primitives match arm enumerated `TextEditCursorLeft` / `Right` / `Up` / `Down` / `Home` / `End` but not the new shift-extend siblings; the `_ => Unhandled` catch-all turned every `Shift+Arrow*` keystroke into a silent no-op on WASM. Added the 6 `*Select` variants to the cursor-primitives match arm. **(d) Major: WASM mouse-click-outside parity** — native `event_mouse_click.rs:435` dispatches `Action::TextEditCommit` and **falls through** to `handle_click` so the click target overrides the just-lifted `SectionRange`; WASM `run_wasm/event_mouse_click.rs:332` `return`d immediately after the commit, leaving the lifted `SectionRange` standing even after the user clicked away to a different node / section / canvas. Removed the early `return`, dropped the renderer borrow before fall-through, so the WASM click path now matches native (lift then override-with-click-target). **(e) Minor: kv verb path no-op warning symmetry** — the picker path's `commit_color_picker` Section arm warns when `set_section_text_color_range` returns false (stale handle); the verb path's `apply_section_colours` and `section_font_outcome` silently folded the false return into "color: no change" / "font: no change", indistinguishable from "the new value matched the old value". Added `log::warn!` from both verb-path arms when the range setter returns false. **(f) Minor: `wasm_compatibility` test undercount** — `test_wasm_compatibility_text_edit_primitives_are_compatible` listed the 6 base cursor variants but not the 6 new `*Select` siblings; extended to all 12. **(g) Minor: tracker drift** — N4-C.b.2 row claimed the lift fires on "both commit AND cancel paths"; updated to reflect the cancel-path gating. **5 new tests**: literal-char typing clears anchor (printable, Enter, Tab) × 3, plus the wasm_compatibility extension implicitly pins the 6 new `*Select` variants × 1, plus the existing `lift_anchor_*` tests already pin the lift contract independent of commit/cancel. Workspace 2296 → 2299 tests. | ✅ |
| N4-D | Picker live preview fan-out for range-scoped section text. | ⏳ |
